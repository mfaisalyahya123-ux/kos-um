...
// Auto-generate attendance for June 3 (Wednesday = work day)
...
console.log('✅ Absensi tercatat!');
console.log('📋 Absensi 3 Juni: Semua hadir (7 orang)');
wageTransactions.forEach(tx => {
  console.log(`   ID ${tx.id}: ${tx.description} Rp ${tx.total.toLocaleString('id-ID')}`);
});
console.log('');
console.log(`   Total upah hari ini: Rp ${wageTransactions.reduce((s,t)=>s+t.total,0).toLocaleString('id-ID')}`);
